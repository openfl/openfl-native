...
class File {
	...
	private function set_url (value:String):String {
		...
		if (value == null) {
			...
			#if iphone
			if (StringTools.startsWith (value, nme_get_resource_path ())) {
	...
	#if iphone
	private static var nme_get_resource_path = Lib.load ("nme", "nme_get_resource_path", 0);
	#end
	...
	private static function nme_filesystem_get_special_dir (which:Int):String {
